...
import { FaUserMd, FaCalendarAlt, FaSyringe, FaChartLine, FaDownload } from 'react-icons/fa';
...
const DoctorStats = () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Bác sĩ', 'Chuyên khoa', 'Lượt khám bệnh', 'Lượt tiêm chủng', 'Tổng lượt khám'];
    const csvContent = [
      headers.join(','),
      ...stats.map(stat => [
        `"${stat.doctorName}"`,
    ...
    link.download = `thong-ke-bac-si-${selectedMonth}-${selectedYear}.csv`;
    link.click();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm">Lượt tiêm chủng</p>
              <p className="text-3xl font-bold">{dashboardStats.totalVaccinations || 0}</p>
            </div>
            <FaSyringe className="w-12 h-12 text-purple-200" />
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg shadow-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-orange-100 text-sm">Bác sĩ hoạt động</p>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={stats} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                ...
                <Bar dataKey="vaccinationCount" fill="#82ca9d" name="Tiêm chủng" />
              </BarChart>
            </ResponsiveContainer>
          </div>

          {/* Biểu đồ tròn - Top 8 bác sĩ */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-6">Phân bố lượt khám (Top 8)</h2>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    data={stats.slice(0, 8)}
                    dataKey="totalExaminations"
                    nameKey="doctorName"
                    cx="50%"
                    cy="50%"
                    outerRadius={100}
                    fill="#8884d8"
                    label={({ doctorName, percent }) => `${doctorName}: ${(percent * 100).toFixed(1)}%`}
                  >
                {(Array.isArray(stats) ? stats.slice(0, 8) : []).map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(value) => [value, 'Tổng lượt khám']} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="space-y-3 max-h-72 overflow-y-auto">
                {(Array.isArray(stats) ? stats.slice(0, 10) : []).map((stat, index) => (
                  <div key={stat.doctorId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500">
                        {stat.appointmentCount} KB + {stat.vaccinationCount} TC
                      </p>
                    </div>
                  </div>
                ))}
