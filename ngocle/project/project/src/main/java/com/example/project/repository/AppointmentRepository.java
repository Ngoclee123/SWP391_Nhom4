...
//    List<Appointment> findRecentAppointments(org.springframework.data.domain.Pageable pageable);
    ...
    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM Appointment a WHERE a.doctor.id = :doctorId AND a.patient.parent.id = :parentId AND a.status = :status")
    boolean existsByDoctorIdAndParentIdAndStatus(@Param("doctorId") Integer doctorId, @Param("parentId") Integer parentId, @Param("status") String status);

    @Query("SELECT a FROM Appointment a WHERE a.patient.parent.id = :parentId ORDER BY a.appointmentDate DESC")
    List<Appointment> findByParentId(@Param("parentId") Integer parentId);


//    @Query("SELECT a FROM Appointment a WHERE a.patient.id = :patientId ORDER BY a.appointmentDate DESC")
//    Page<Appointment> findByPatientIdOrderByAppointmentDateDesc(@Param("patientId") Integer patientId, Pageable pageable);


    @Query("SELECT a FROM Appointment a WHERE a.patient.parent.id = :parentId AND a.doctor.id = :doctorId AND a.status = :status ORDER BY a.appointmentDate DESC")
    List<Appointment> findByParentIdAndDoctorIdAndStatus(@Param("parentId") Integer parentId, @Param("doctorId") Integer doctorId, @Param("status") String status);


    @Query("SELECT a FROM Appointment a WHERE a.patient.parent.account.id = :accountId AND a.doctor.id = :doctorId AND a.status = :status ORDER BY a.appointmentDate DESC")
    List<Appointment> findByAccountIdAndDoctorIdAndStatus(@Param("accountId") Integer accountId, @Param("doctorId") Integer doctorId, @Param("status") String status);
