...
const token = UserService.getToken();
            ...
            setMessageHistory(prev => ({
                ...prev,
                [doctorUsername]: Array.isArray(historyMessages)
...
for (let i = 0; i < sender.length; i++) {
    ...
    return (
        <div className="fixed bottom-24 right-8 z-[9999]">
            <button
                className={`relative w-14 h-14 flex items-center justify-center bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-full shadow-xl hover:shadow-2xl hover:scale-110 transform transition-all duration-300 ${isChatOpen ? 'rotate-180' : ''}`}
...
className={`w-6 h-6 absolute top-0 left-0 transition-opacity duration-300 ${isChatOpen ? 'opacity-100' : 'opacity-0'}`}
            ...
            {isChatOpen && (
                <div className="w-80 sm:w-96 h-[550px] bg-white rounded-2xl shadow-2xl flex flex-col fixed bottom-24 right-8 overflow-hidden border border-gray-100 animate-in slide-in-from-bottom-5 duration-300 z-[9999]">
                    {/* Header */}
