...
axiosClient.interceptors.request.use(async (req) => {
  ...
  if (
    token &&
    !req.url.match(
      /\/api\/(login|register|auth\/google\/callback|oauth2\/authorization\/google|login\/oauth2\/code\/|login|oauth2\/redirect|vnpay|doctors(?!\/\d+)|vaccine-appointments\/availability)/
    )
  ) {
    req.headers = req.headers || {};
    req.headers.Authorization = `Bearer ${token}`;
...
const handleOAuthRedirect = () => {
  ...
  if (token && role) {
    ...
    console.log(
      "OAuth redirect: Token set, Role:",
      role,
      "Redirecting to:",
      role.toLowerCase() === "user" ? "/home" : "/login"
    );
    if (role.toLowerCase() === "user") {
      window.location.href = "/home";
    } else {
